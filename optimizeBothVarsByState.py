...
print(data['Population'].quantile([0.25]))
...
for i in tqdm(states):
    start = datetime.now()
    data2 = data.loc[data['State']==i].sort_values('Estimated_annual_rent')
...
result_df['Demand'] = result_df.apply(lambda x: demand(x['NStores'], x['Prices'], x['Population']),axis=1)
result_df['Revenue'] = result_df.apply(lambda x: revenue(x['Prices'], x['MeanIncomeRatio'], x['Demand']),axis=1)
result_df.loc[result_df['Revenue'] <= 0,'Revenue'] = 0
result_df.loc[result_df['Revenue'] <= 0,'Prices'] = 0
result_df.loc[result_df['Profit'] < 0,'Profit'] = 0
result_df['LogProfit'] = np.log(result_df['Profit']+1)
...
plt.savefig(f"Images/Optimal_Price_X_by_State_1e7_N10.png",bbox_inches='tight',dpi=300)
plt.show()
plt.close()
...
cbar = plt.cm.ScalarMappable(norm=norm, cmap='viridis')
plt.colorbar(cbar, ax=ax, pad = -0.25, shrink = 0.4)
plt.title(f'Log of Profit By County', y = .8, fontsize='small')
plt.savefig(f"Images/Optimal_Profit_X_Price_by_State_1e7_N10.png",bbox_inches='tight',dpi=300)
plt.show()
plt.close()
...
cbar = plt.cm.ScalarMappable(norm=norm, cmap='viridis')
plt.colorbar(cbar, ax=ax, boundaries=np.arange(0,N,int(N/10)), pad = -0.25, shrink = 0.4)
plt.title(f'Number of Stores By County', y = .8, fontsize='small')
plt.savefig(f"Images/Optimal_NStores_2vars_by_State_1e7_N10.png",bbox_inches='tight',dpi=300)
plt.show()
plt.close()
...
for state in states:
    df = result_df.loc[result_df['State']==state]
    print(state)
    print(df[['NStores','Profit']].describe())

state_results = result_df.groupby(['State'])['Profit'].sum()
print(state_results)

profitable = result_df.loc[(result_df['Profit']>0)&(result_df['NStores']>=1)]
print("Mean Revenue",profitable.groupby(['State'])['Revenue'].mean())
print("Mean Profit Margin",profitable.groupby(['State'])['Profit_Margin'].mean())
print("Mean Prices",profitable.groupby(['State'])['Prices'].mean())
print("Number of Stores",profitable.groupby(['State'])['NStores'].sum())

print(profitable.loc[profitable['NStores']>=4])

print("Minimum and Maximum optimal price:",profitable['Prices'].min(),profitable['Prices'].max())

print("Columns of profitable dataset",profitable.columns)
rural = profitable.loc[profitable['Population']<=5000]
urban = profitable.loc[profitable['Population']>5000]

print('Percent of counties with stores that are rural:',rural.shape[0]/(rural.shape[0]+urban.shape[0])*100)
print('Percent of counties with stores that are urban:',urban.shape[0]/(rural.shape[0]+urban.shape[0])*100)

print("Rural Prices:\n",rural['Prices'].describe())
print("Urban Prices:\n",urban['Prices'].describe())

print("Rural Profit:\n",rural['Profit'].describe())
print("Urban Profit:\n",urban['Profit'].describe())

low_income_thresh = data['MeanIncome'].quantile([0.25]).values[0]
low_income = profitable.loc[profitable['MeanIncome']<=low_income_thresh]
high_income = profitable.loc[profitable['MeanIncome']>low_income_thresh]

print("Percent of counties with stores below low income threshold",low_income.shape[0]/(low_income.shape[0]+high_income.shape[0])*100)
print("Percent of counties with stores above low income threshold",high_income.shape[0]/(low_income.shape[0]+high_income.shape[0])*100)

print("Low income county Prices:\n",low_income['Prices'].describe())
print("High income county Prices:\n",high_income['Prices'].describe())

print("Low income county Profit:\n",low_income['Profit'].describe())
print("High income county Profit:\n",high_income['Profit'].describe())
exit()
#data['Nstores'] = x[:100]
