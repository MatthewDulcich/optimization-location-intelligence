...
# from cupyx.optimizing import optimize as opt
import cupy as cp
import cvxpy
...
#     print("CUDA is available and detected by CuPy.")
# except cp.cuda.runtime.CUDARuntimeError as e:
#     print("CUDA is not available or not properly configured:", e)
...
def demand(x, P, totalPop):
    ...
    Args:
        x: Total number of stores in given county (can be symbolic).
        P: Price.
        totalPop: Total population in county.
    ...
    L = cp.where(x >= 1, totalPop / x, 0)  # Use cp.where to handle symbolic variables
    L = cp.where(P > 50, 0, L)  # Set L to 0 if P > 50
    return L * cp.exp(-a * P)
...
def getConstraints(x, budget, N, risk, totalPop, IR, minwage, rent):
    ...
    def minimum_price(x):
        return x[NUMBER_OF_COUNTIES:] - 10
    ...
    return [
        ...
        {'type': 'ineq', 'fun': lambda x: minimum_price(x)}, # All x, P > 0
    ]
    #return [
...
def cupy_optimize(budget, N, risk, totalPop, IR, minwage, rent):
    ...
    x = cvxpy.Variable(NUMBER_OF_COUNTIES)
    P = cvxpy.Variable(NUMBER_OF_COUNTIES)
    budget = cvxpy.Parameter(budget)
    N = cvxpy.Parameter(N)
    risk = cvxpy.Parameter(risk)
    totalPop = cvxpy.Parameter(totalPop)
    IR = cvxpy.Parameter(IR)
    minwage = cvxpy.Parameter(minwage)
    rent = cvxpy.Parameter(rent)
    ...
    def risk_constraint(x):
        ...
        return [(risk - cost/rev) if r != 0 else -1 for c, r in zip(cost,rev)]
